...
namespace Cross.BluetoothLe.Helpers
{
  public static class TaskBuilder
  {
    /// <summary>Main thread queue get semaphore timeout</summary>
    ...
    public static async Task<TReturn> FromEventAsync<TReturn, TEventHandler, TRejectHandler>(
      Action execute,
      ...
      CancellationToken token = default)
    {
      ...
      try
      {
        ...
        {
          return await SafeEnqueueAndExecuteAsync(execute, token, tcs);
        }
      }
      finally
      {
    ...
    public static Task EnqueueOnMainThreadAsync(Action execute, CancellationToken token = default)
       => SafeEnqueueAndExecuteAsync<bool>(execute, token);

    private static async Task<TReturn> SafeEnqueueAndExecuteAsync<TReturn>(Action execute, CancellationToken token, TaskCompletionSource<TReturn> tcs = null)
    {
