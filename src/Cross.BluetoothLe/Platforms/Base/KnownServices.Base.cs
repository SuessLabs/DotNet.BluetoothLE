...
namespace Cross.BluetoothLe
{
  ...
  {
    private static readonly Dictionary<Guid, KnownService> References;

    static KnownServices()
    {
      References = Services.ToDictionary(s => s.Id, s => s);
    }
    ...
    public static KnownService Find(Guid id)
    {
      return References.ContainsKey(id) ? References[id] : new KnownService("Unknown Service", Guid.Empty);
    }
  }
}
